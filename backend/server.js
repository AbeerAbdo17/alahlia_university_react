...
app.get('/api/students/nationalities', async (req, res) => {
    try {
        const [rows] = await dbp.execute(
            `SELECT DISTINCT nationality FROM students WHERE nationality IS NOT NULL AND nationality != '' ORDER BY nationality ASC`
        );
        const list = rows.map(r => r.nationality);
        res.json(list);
    } catch (err) {
...
app.post("/api/term-results/calculate-save", async (req, res) => {
  ...
  try {
    ...
    // ✅ دالة حساب التراكمي الصحيح (للفترة المحددة فقط)
    async function calcCumulativeGpa(studentId) {
      try {
        // 1. قائمة المواد المعادة في الفصل الحالي
        ...
        // 2. جلب كل الدرجات حتى الفترة الحالية فقط
        const [gradeRows] = await conn.query(
          ...
          WHERE cg.student_id = ?
            ...
            AND (
              c.academic_year < ?
              OR (c.academic_year = ? AND c.level_name <= ? AND FIELD(TRIM(c.term_name), ${termVariants.map(() => "?").join(",")}) > 0)
            )
          ORDER BY 
            c.academic_year ASC,
            CASE 
    ...
    for (const st of students) {
      ...
      let termSumPH = 0;
      let termSumH = 0;
      let failedHours = 0;  // ✅ إضافة متغير لحساب ساعات الرسوب
      ...
      for (const c of courses) {
        ...
        // ✅ حساب ساعات الرسوب (الدرجة < 60)
        if (Number(g.total_mark) < 49) {
          failedHours += h;
        }
      }
      ...
      if (existing.length > 0) {
        // ✅ تحديث النتيجة مع failed_hours المحسوبة بشكل صحيح
        await conn.query(
          `UPDATE term_results SET 
             ...
             updated_at = NOW()
           WHERE id = ?`,
          [termGpa, cumulativeGpa, termSumPH, termSumH, failedHours, classificationLabel,
           coursesCount, completed, missing, existing[0].id]
        );
      } else {
        // ✅ إدراج نتيجة جديدة مع failed_hours
        await conn.query(
          `
          ...
          [
            ...
            termSumPH, termSumH, failedHours,
            classificationLabel,
  ...
  try {
    ...
    const [rows] = await dbp.query(
      ...
      SELECT DISTINCT
        ...
        COALESCE(f.installment_4_paid, 0) as inst4,
        COALESCE(f.installment_5_paid, 0) as inst5,
        COALESCE(f.installment_6_paid, 0) as inst6
      FROM students s
      INNER JOIN student_registrations sr ON sr.student_id = s.id
      LEFT JOIN fees f ON f.student_id = s.id AND f.academic_year = sr.academic_year
      WHERE s.department_id = ?
        AND sr.academic_year = ?
        AND sr.level_name = ?
        AND sr.term_name = ?
...
app.put('/api/users/:id', async (req, res) => {
  ...
  try {
    ...
    console.error("حدث خطأ في PUT /api/users/:id :", err);
...
app.get("/api/term-default-fees", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ error: "لا توجد رسوم افتراضية لهذا التخصص" });
    }

    res.json(rows[0]);
  } catch (err) {
    console.error("Get term default fees error:", err);
    res.status(500).json({ error: "خطأ في جلب الرسوم من السيرفر" });
  }
});
